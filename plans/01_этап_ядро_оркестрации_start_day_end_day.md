...
- `run_steps(steps: list[Callable]) -> list[StepExecutionReport]`
- Нужен, чтобы не дублировать одинаковую логику:
  - запуск шага;
  - замер времени;
  - фиксация ошибки/успеха;
  - единый `StepExecutionReport`.
  - фиксированное поведение MVP: при ошибке шага формируем отчет и продолжаем pipeline, где это безопасно.
...
## Границы MVP этапа 01

- `fail_fast`, retry-стратегии, продвинутые режимы оркестрации и дополнительные флаги **не включаем** в этап 01.
- Реализуем только линейный и понятный pipeline без ветвления по режимам.
- Любые усложнения и расширения фиксируем как post-MVP задачи и рассматриваем только после стабилизации базового цикла `start_day` / `end_day`.
